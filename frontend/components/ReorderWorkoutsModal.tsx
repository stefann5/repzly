import { useState, useEffect } from "react";
import { Modal, View, Pressable, ScrollView } from "react-native";
import { Ionicons } from "@expo/vector-icons";
import { Label } from "@/components/Label";
import { Button } from "@/components/Button";
import { WorkoutGroup } from "@/types/program";

interface ReorderWorkoutsModalProps {
  visible: boolean;
  workouts: WorkoutGroup[];
  onClose: () => void;
  onSave: (workoutNumbers: number[]) => void;
}

export function ReorderWorkoutsModal({
  visible,
  workouts,
  onClose,
  onSave,
}: ReorderWorkoutsModalProps) {
  const [orderedWorkouts, setOrderedWorkouts] = useState<WorkoutGroup[]>([]);

  useEffect(() => {
    if (visible) {
      setOrderedWorkouts([...workouts]);
    }
  }, [visible, workouts]);

  const moveUp = (index: number) => {
    if (index === 0) return;
    const newOrder = [...orderedWorkouts];
    [newOrder[index - 1], newOrder[index]] = [newOrder[index], newOrder[index - 1]];
    setOrderedWorkouts(newOrder);
  };

  const moveDown = (index: number) => {
    if (index === orderedWorkouts.length - 1) return;
    const newOrder = [...orderedWorkouts];
    [newOrder[index], newOrder[index + 1]] = [newOrder[index + 1], newOrder[index]];
    setOrderedWorkouts(newOrder);
  };

  const handleSave = () => {
    onSave(orderedWorkouts.map((w) => w.workout_number));
    onClose();
  };

  const handleCancel = () => {
    setOrderedWorkouts([...workouts]);
    onClose();
  };

  return (
    <Modal
      visible={visible}
      animationType="slide"
      presentationStyle="pageSheet"
      onRequestClose={handleCancel}
    >
      <View className="flex-1 bg-white dark:bg-zinc-900">
        {/* Header */}
        <View className="px-4 py-4 border-b border-gray-200 dark:border-zinc-700">
          <Label variant="heading" weight="bold" styleClass="text-center">
            Reorder Workouts
          </Label>
          <Label variant="caption" color="secondary" styleClass="text-center mt-1">
            Use arrows to change order
          </Label>
        </View>

        {/* Content */}
        <ScrollView className="flex-1 px-4">
          {orderedWorkouts.map((workout, index) => {
            const totalSets = workout.exercises.reduce((sum, ex) => sum + ex.sets.length, 0);
            return (
              <View
                key={workout.workout_number}
                className="flex-row items-center p-3 my-1 bg-gray-100 dark:bg-zinc-800 rounded-lg"
              >
                <View className="w-8 h-8 items-center justify-center bg-blue-500 rounded-full mr-3">
                  <Label variant="body" weight="semibold" styleClass="text-white">
                    {index + 1}
                  </Label>
                </View>
                <View className="flex-1">
                  <Label variant="body" weight="medium">
                    Workout
                  </Label>
                  <Label variant="caption" color="secondary">
                    {workout.exercises.length} exercise{workout.exercises.length !== 1 ? "s" : ""} • {totalSets} set{totalSets !== 1 ? "s" : ""}
                  </Label>
                </View>
                <View className="flex-row">
                  <Pressable
                    onPress={() => moveUp(index)}
                    disabled={index === 0}
                    className="p-2"
                  >
                    <Ionicons
                      name="chevron-up"
                      size={24}
                      color={index === 0 ? "#D1D5DB" : "#3b82f6"}
                    />
                  </Pressable>
                  <Pressable
                    onPress={() => moveDown(index)}
                    disabled={index === orderedWorkouts.length - 1}
                    className="p-2"
                  >
                    <Ionicons
                      name="chevron-down"
                      size={24}
                      color={index === orderedWorkouts.length - 1 ? "#D1D5DB" : "#3b82f6"}
                    />
                  </Pressable>
                </View>
              </View>
            );
          })}
        </ScrollView>

        {/* Footer */}
        <View className="flex-row justify-between px-4 py-4 border-t border-gray-200 dark:border-zinc-700">
          <Button
            title="Cancel"
            theme="secondary"
            onPress={handleCancel}
            styleClass="flex-1 mr-2"
          />
          <Button
            title="Ok"
            theme="primary"
            onPress={handleSave}
            styleClass="flex-1 ml-2"
          />
        </View>
      </View>
    </Modal>
  );
}
